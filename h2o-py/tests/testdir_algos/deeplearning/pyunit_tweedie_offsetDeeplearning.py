...
def tweedie_offset(ip,port):
    ...
    assert abs(47.61-predictions.mean()[0]) < 1e-2, "Expected mean of predictions to be 47.61, but got " \
                                                          "{0}".format(predictions[0].mean())
    assert abs(1.94-predictions.min()) < 1e-1, "Expected min of predictions to be 1.94, but got " \
                                                          "{0}".format(predictions[0].min())
    assert abs(284.6-predictions.max()) < 28, "Expected max of predictions to be 284.6, but got " \
    ...
    assert abs(0.261-mean_residual_deviance) < 1e-2, "Expected mean residual deviance to be 0.261, but got " \
                                                         "{0}".format(mean_residual_deviance)
    predictions = dl.predict(insurance)
    assert abs(49.53-predictions.mean()[0]) < 1e-1, "Expected mean of predictions to be 49.53, but got " \
                                                          "{0}".format(predictions[0].mean())
    assert abs(1.074-predictions.min()) < 1e-1, "Expected min of predictions to be 1.074, but got " \
                                                          "{0}".format(predictions[0].min())
    assert abs(397.3-predictions.max()) < 40, "Expected max of predictions to be 397.3, but got " \
                                                          "{0}".format(predictions[0].max())
