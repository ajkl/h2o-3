...
def init_err_casesKmeans(ip,port):
    ...
    try:
        h2o.kmeans(x=benign_h2o, k=5, user_points=h2o.H2OFrame(zip(*start_small)))
        assert False, "expected an error"
    except EnvironmentError:
        assert True

    try:
        h2o.kmeans(x=benign_h2o, k=5, user_points=h2o.H2OFrame(zip(*start_large)))
        assert False, "expected an error"
    except EnvironmentError:
        assert True
    ...
    try:
        h2o.kmeans(x=benign_h2o, k=numrow+2, user_points=h2o.H2OFrame(zip(*start)))
        assert False, "expected an error"
    except EnvironmentError:
        assert True
    ...
    for x in ["NA", "NaN", "Inf", "-Inf"]:
        ...
        h2o.kmeans(x=benign_h2o, k=3, user_points=h2o.H2OFrame(zip(*start_err)))

    # Duplicates will affect sampling probability during initialization.
    # Log.info("Duplicate initial clusters specified")
    start = [[random.gauss(0,1) for c in range(numcol)] for r in range(3)]
    start[2] = start[0]
    h2o.kmeans(x=benign_h2o, k=3, user_points=h2o.H2OFrame(zip(*start)))
